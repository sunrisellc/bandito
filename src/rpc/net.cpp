    ...
    if (request.fHelp || request.params.size() != 2 ||
        ...
        throw std::runtime_error(
            RPCHelpMan{"addnode",
                ...
                RPCResults{},
                RPCExamples{
                    HelpExampleCli("addnode", "\"192.168.0.6:4269\" \"onetry\"")
            + HelpExampleRpc("addnode", "\"192.168.0.6:4269\", \"onetry\"")
                },
            }.ToString());
...
static UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:4269\",  (string) The bandito server IP and port we're connected to\n"
